...
func TestNewProcessesCollector(t *testing.T) {
	...
	{
		c, err := NewProcessesCollector(filepath.Join("testdata", "config_report_all_cpus_true_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if !c.(*Processes).reportAllCPUs {
	...
	{
		c, err := NewProcessesCollector(filepath.Join("testdata", "config_report_all_cpus_false_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if c.(*Processes).reportAllCPUs {
