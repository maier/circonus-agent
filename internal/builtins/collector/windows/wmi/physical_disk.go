...
// PhysicalDisk metrics from the Windows Management Interface (wmi)
...
// physicalDiskOptions defines what elements can be overriden in a config file
type physicalDiskOptions struct {
...
// NewPhysicalDiskCollector creates new wmi collector
func NewPhysicalDiskCollector(cfgBaseName string) (collector.Collector, error) {
	c := PhysicalDisk{}
	c.id = "physical_disk"
	...
	if cfg.MetricsDefaultStatus != "" {
		if ok, _ := regexp.MatchString(`^(enabled|disabled)$`, strings.ToLower(cfg.MetricsDefaultStatus)); ok {
			...
			return nil, errors.Errorf("wmi.physical_disk invalid metric default status (%s)", cfg.MetricsDefaultStatus)
		}
	}
	...
	if cfg.MetricNameRegex != "" {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "wmi.physical_disk compile metric_name_regex")
		}
	...
	if cfg.RunTTL != "" {
		...
		if err != nil {
			return nil, errors.Wrap(err, "wmi.physical_disk parsing run_ttl")
		}
		c.runTTL = dur
	}
...
func (c *PhysicalDisk) Collect() error {
	metrics := cgm.Metrics{}
	...
	if err := wmi.Query(qry, &dst); err != nil {
		...
		return errors.Wrap(err, "wmi.physical_disk")
	}
